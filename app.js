/* ── RCE Sandbox frontend: editor + websocket + terminal ── */
...
function registerPythonCompletions() {
  const kw = (label) => ({ label, kind: monaco.languages.CompletionItemKind.Keyword, insertText: label });
  ...
  const suggestions = [
    ...
    ...["and", "as", "assert", "async", "await", "break", "class", "continue", "def", "del",
      ...
      "with", "yield", "True", "False", "None"].map(kw),
    ...
    fn("sorted", "sorted(iterable, key=None, reverse=False)", "Return a sorted list."),
    ...
    snip("with", "with open(${1:'file.txt'}, ${2:'r'}) as ${3:f}:\n    ${4:pass}", "with open(...)"),
    snip("lambda", "lambda ${1:x}: ${2:x}", "lambda expression"),
    snip("listcomp", "[${1:x} for ${1:x} in ${2:iterable}]", "list comprehension"),
  ];
